...
export function LeadDrawer({ open, onClose, lead, slug }: LeadDrawerProps) {
  const navigate = useNavigate();
  const { updateLead, selectConversation, conversations } = useMultiTenantStore();
  ...
  const handleAddTag = () => {
    if (newTag.trim() && editedLead.tags) {
      setEditedLead({ ...editedLead, tags: [...editedLead.tags, newTag.trim()] });
      setNewTag('');
    }
  };
  ...
  const handleRemoveTag = (tagToRemove: string) => {
    if (editedLead.tags) {
      setEditedLead({ ...editedLead, tags: editedLead.tags.filter(tag => tag !== tagToRemove) });
    }
  };
      ...
      >
        ...
        <div className="p-6 space-y-6 animate-slide-right">
          <div className="flex items-center gap-4">
            <img src={lead.avatar} alt={lead.name} className="w-20 h-20 rounded-full border-2 border-primary/30" />
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2"><Phone className="w-4 h-4 text-muted-foreground" />Telefone</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2"><Building className="w-4 h-4 text-muted-foreground" />Empresa</Label>
              <Input
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2"><DollarSign className="w-4 h-4 text-muted-foreground" />Valor</Label>
              <Input
                type="number"
          ...
          <div className="space-y-3">
            <Label className="flex items-center gap-2"><Tag className="w-4 h-4 text-muted-foreground" />Tags</Label>
            <div className="flex flex-wrap gap-2">
              {editedLead.tags?.map((tag) => (
                <Badge key={tag} variant="secondary" className="cursor-pointer hover:bg-destructive/20" onClick={() => handleRemoveTag(tag)}>
                  {tag} ×
                </Badge>
              ))}
            </div>
          ...
          <div className="p-4 rounded-lg bg-secondary/50 space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span className="text-foreground">{new Date(lead.createdAt).toLocaleDateString('pt-BR')}</span>
            </div>
            <div className="flex justify-between text-sm">
